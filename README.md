...
<body>
    ...
    <script type="module">
        ...
        async function loadData(uid) {
            ...
            // CORRECTION: Ensure we map with the correct document ID for deletions
            const trashQuery = query(collection(db, "trash"), where("userId", "==", uid));
            onSnapshot(trashQuery, (snap) => {
                trash = snap.docs.map(d => ({ ...d.data(), id: d.id })); // Ensure ID is the Firestore Doc ID
                renderTrash();
            });
        ...
        window.restore = async (id) => {
            ...
            if (!item) return;
            try {
                // Copy all data except 'id' and 'deletedAt'
                // This ensures we create a NEW subscriber doc and delete the OLD trash doc
                const { id: trashDocId, deletedAt, ...subscriberData } = item;
